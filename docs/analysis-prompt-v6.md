...
## Objective Summary vs Preference Verdict

The model must separate objective reporting from subjective recommendation.

Step 1: Generate the objective report lists from Release Notes before applying user preferences:
...
User preferences must not reduce, hide, or delete meaningful facts from these lists.

Step 2: Use the objective lists plus user preferences to decide:

- `verdict`
- `verdictReason`

Preference weighting applies only to the final verdict and verdict reason.

- A category marked `strong` should carry much more weight.
- A category marked `ignore` must still be objectively summarized when meaningful facts exist.
- A category marked `ignore` should not by itself push the verdict to `yes`.
...
Then generate objective factual lists first. Do not use user preferences to filter these lists.
...
- `criticalFixes`:
  bug and vulnerability fixes only. Include security fixes, crash fixes, data issues, privacy issues, core-flow availability fixes, and important bugs that clearly affect target users. Do not include routine maintenance, test fixes, or development-dependency fixes.
- `breakingChanges`:
...
Important: if the user marks bug fixes as `ignore`, still list meaningful bug, crash, security, data, privacy, or core-flow fixes in `criticalFixes`. The `ignore` preference only means ordinary bugs should not by themselves push the final verdict to `yes`.
...
These high-risk rules affect recommendation strength, not whether a meaningful fix may be listed. Important non-high-risk fixes can still appear in `criticalFixes`; they simply should not be described as mandatory or high-risk.
...
Use this guidance after the objective report lists have been generated:
